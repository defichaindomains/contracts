...
async function main() {
  ...
  const BaseRegistrarContract = await hre.ethers.getContractFactory(
    'BaseRegistrarImplementation',
  )
  const registrar = await BaseRegistrarContract.deploy(
    ens.address,
    hre.ethers.utils.namehash(TLD),
  )
  ...
  const dummy = await DummyOracleContract.deploy(
    hre.ethers.BigNumber.from('200000000000'),
  )
  await dummy.deployed()
  console.log('DummyOracle deployed to:', dummy.address)

  // Deploying StablePriceOracle
  const PriceOracleContract = await hre.ethers.getContractFactory(
    'StablePriceOracle',
  )
  ...
  const ControllerContract = await hre.ethers.getContractFactory(
